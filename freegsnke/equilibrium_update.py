...
class Equilibrium(freegs4e.equilibrium.Equilibrium):
    ...
    def adjust_psi_plasma(
        ...
        if opoint_flag == False:
            logger.warning(
                "O-point could not be generated by simply scaling up psi_plasma. Manual initialization advised."
            )
            return

        # O-point is in place
        xpoint_flag = len(xpt) > 0
        logger.info("O-point is in place. Flag for X-point in place = %s", xpoint_flag)
        ...
        if (xpoint_flag == False) and (n_exp < 10):
            ...
            while (xpoint_flag == False) and (n_exp < 10):
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to introduce an xpoint on the domain by exponentiating psi_plasma. Manual initialization advised."
                    )
                    return
        ...
        if xpoint_flag == False:
            ...
            while diverted_flag == False and n_up < 6:
                ...
                if xpoint_flag:
                    ...
                    diverted_size = np.sum(diverted_core_mask)
                    logger.debug("diverted_size: %s", diverted_size)
                # except:
                #     diverted_flag = True
    ...
    def psi_func(self, R, Z, *args, **kwargs):
        ...
        if check:
            logger.debug(
                "Discrepancy between psi_func and plasma_psi detected. psi_func has been reset."
            )
            # redefine interpolating function
            self.psi_func_interp = interpolate.RectBivariateSpline(
                self.R[:, 0], self.Z[0, :], self.plasma_psi
