...
class evolve_plasma_NK:
    ...
    def __init__(self, profiles, eq):
        ...
        self.eq1 = freegs.Equilibrium(
            tokamak=eq.getMachine(),
            Rmin=eq.R[0, 0],
            Rmax=eq.R[-1, -1],  # Radial domain
            Zmin=eq.Z[0, 0],
            Zmax=eq.Z[-1, -1],  # Height range
            nx=np.shape(eq.R)[0],
            ny=np.shape(eq.R)[1],  # Number of grid points
            psi=eq.plasma_psi,
        )
        self.profiles1 = ConstrainPaxisIp(
            self.paxis,  # Plasma pressure on axis [Pascals]
        ...
        self.profiles2 = ConstrainPaxisIp(
            self.paxis,  # Plasma pressure on axis [Pascals]
            eq.plasmaCurrent(),  # Plasma current [Amps]
            self.fvac,  # vacuum f = R*Bt
            alpha_m=self.alpha_m,
        ...
        mask_inside_reactor *= (eq.Z < 0.96 + 1 * eq.R) * (eq.Z > -0.96 - 1 * eq.R)
        mask_inside_reactor *= (eq.Z < -2 + 9.0 * eq.R) * (eq.Z > 2 - 9.0 * eq.R)
        ...
        invm = np.linalg.inv(
            self.evol_currents.R_matrix[:-1, :-1] + machine_config.coil_self_ind / 0.001
        )
        v, w = np.linalg.eig(np.matmul(invm, machine_config.coil_self_ind / 0.001))
        w = w[:, np.argsort(-v)[:50]]
    ...
    def find_dt_evolve(self, U_active, max_rel_change, results=None, dR=0):
        ...
        while np.sum(abs(rel_change_curr) > max_rel_change):
            dt_step /= 1.5
            new_currents = self.evol_currents.new_currents_out(
                self.eq1, self.results, U_active, dt_step, dR
            )
            rel_change_curr = (
                abs(new_currents - self.currents_vec) / self.vals_for_rel_change
            )
    ...
    def update_R_matrix(self, trial_currents, rtol_NK=1e-6):  # , verbose_NK=False):
        ...
        dLpc = (
            self.results1["plasma_ind_on_coils"] - self.results["plasma_ind_on_coils"]
        )
        dLpc /= self.dt_step
    ...
    def Fcircuit(self, trial_currents, rtol_NK=1e-7):
        # ,  verbose_NK=False):
        self._dR = self.update_R_matrix(trial_currents, rtol_NK)  # , verbose_NK)
        new_currents = self.evol_currents.stepper_adapt_repeat(
            self.currents_vec, dR=self._dR
        )
    ...
    def dI(self, res0, G, Q, clip=5):
        ...
        self.coeffs = np.matmul(np.matmul(np.linalg.inv(np.matmul(G.T, G)), G.T), -res0)
    ...
    def Arnoldi_iter(
        ...
        conv_crit=0.5,  # add basis vector
        # if orthogonal residual is larger than
        grad_eps=0.001,  # relative magnitude of infinitesimal step, with respect to self.vals_for_rel_change
        # adjust in line to 0.1*max_rel_change
        accept_threshold=0.2,
        max_collinearity=0.6,
        clip=1.5,
        # ,verbose_currents=False
    ):
        ...
        # use at least 1 orthogonal terms, but not more than n_k
        failure_count = 0
        while ((n_it < 1) + (arnoldi_control > 0)) * (n_it < n_k) * (
            (failure_count < 2) + (n_it < 1)
        ) > 0:

            # print('failure_count', failure_count)
            nvec_direction = np.linalg.norm(vec_direction)
            grad_coeff = grad_eps * ntrial_currents / nvec_direction * nFresidual / 0.01
            ...
            if ((di_factor < 0.3) * (abs(candidate_di[-1]) < 150)) + (di_factor > 6):
            ...
            if costerm > accept_threshold:
                collinearity = (
                    np.sum(dcandidate_usable[:, np.newaxis] * Gn[:, :n_it], axis=0)
                    > max_collinearity
                )
                if np.sum(collinearity):
                    ...
                    self.dI(Fresidual, G=self.G, Q=self.Q, clip=clip)
                    rel_unexpl_res = (
                        np.linalg.norm(self.eplained_res + Fresidual) / nFresidual
                    )
                    # print('relative_unexplained_residual = ', rel_unexpl_res)
                    arnoldi_control = rel_unexpl_res > conv_crit
    ...
    def do_step(
        self,
        U_active,  # active potential
        max_rel_residual=0.001,  # aim for a dt such that the maximum relative change in the currents is max_rel_change
        max_dt_step=0.0002,  # do multiple timesteps with constant \dotL if larger than
        rtol_NK=1e-6,  # for convergence of the NK solver of GS
        # verbose_NK=False,
        ...
        max_iter=10,  # if more iterative steps are required, dt is reduced
        n_k=10,  # maximum number of terms in Arnoldi expansion
        conv_crit=0.15,  # add more Arnoldi terms if residual is still larger than
        grad_eps=0.00003,
        clip=1.5,
    ):  # relative magnitude of infinitesimal step, with respect to self.vals_for_rel_change
        # adjust in line to 0.1*max_rel_change
        ...
        abs_currents = abs(self.currents_vec)
        self.vals_for_rel_change = np.where(
            abs_currents > self.threshold, abs_currents, self.threshold
        )
        ...
        while not_done_flag:
            ...
            self.trial_currents = self.evol_currents.stepper_adapt_first(
                self.currents_vec, U_active, self.dR
            )
            Fresidual = self.Fcircuit(self.trial_currents, rtol_NK)  # , verbose_NK)
            rel_change = abs(Fresidual)
            max_rel_change = max(rel_change)
            if max_rel_change < 0.6 * max_rel_residual:
                # print('increasing timestep')
                self.dt_step *= min(3, (0.8 * max_rel_residual / max_rel_change))
                self.evol_currents.determine_stepsize(self.dt_step, max_dt_step)
                self.trial_currents = self.evol_currents.stepper_adapt_repeat(
                    self.currents_vec, self.dR
                )
                Fresidual = self.Fcircuit(self.trial_currents, rtol_NK)  # , verbose_NK)
                rel_change = abs(Fresidual)
                max_rel_change = max(rel_change)
            while (max_rel_change > max_rel_residual) * (self.dt_step > 5e-5):
                ...
                self.dt_step *= 0.8 * max_rel_residual / max_rel_change
                self.evol_currents.determine_stepsize(self.dt_step, max_dt_step)
                self.trial_currents = self.evol_currents.stepper_adapt_first(
                    self.currents_vec, U_active, self.dR
                )
            ...
            while it < max_iter and not_done_flag:
                if max_rel_change < rtol_currents:
                    ...
                    # nFresidual = np.linalg.norm(Fresidual)
                    used_n = self.Arnoldi_iter(
                        self.trial_currents,
                        Fresidual
                        * self.vals_for_rel_change,  # starting direction in I space
        ...
        self.plasma_against_wall = np.sum(
            self.mask_outside_reactor * self.results["separatrix"][1]
        )
        return self.plasma_against_wall
