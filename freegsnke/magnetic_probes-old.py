...
class quants_for_emulation:
    ...
    def __init__(self, eq, probe_dict=probe_dict):
        ...
        red_idx = np.round(
            np.array(np.shape(eq.R)) * np.array([[12, 100], [30, 99]]) / 129
        ).astype(int)
        red_R = eq.R[red_idx[0, 0] : red_idx[0, 1], red_idx[1, 0] : red_idx[1, 1]]
        red_Z = eq.Z[red_idx[0, 0] : red_idx[0, 1], red_idx[1, 0] : red_idx[1, 1]]
        self.red_idx = red_idx

        # for plasma-plasma flux
        greenmatrix = Greens(
            red_R[:, :, np.newaxis, np.newaxis],
            red_Z[:, :, np.newaxis, np.newaxis],
            red_R[np.newaxis, np.newaxis, :, :],
            red_Z[np.newaxis, np.newaxis, :, :],
        )
    ...
    def get_pickups(self):
        ...
        for coil in self.pccoils:
            ...
            if tRi < NX - 1 and tRi > 0:
                tRp = (tRi + 1) * (
                    (NX - 1) * (tpR - Rmin) / (Rmax - Rmin) > tRi + 0.5
                ) + tRi * ((NX - 1) * (tpR - Rmin) / (Rmax - Rmin) <= tRi + 0.5)
                tRm = (tRi) * ((NX - 1) * (tpR - Rmin) / (Rmax - Rmin) > tRi + 0.5) + (
                    tRi - 1
                ) * ((NX - 1) * (tpR - Rmin) / (Rmax - Rmin) <= tRi + 0.5)
            elif tRi < NX - 1:
                tRp = tRi
            ...
            if tZi < NY - 1 and tZi > 0:
                tZp = (tZi + 1) * (
                    (NY - 1) * (tpZ - Zmin) / (Zmax - Zmin) > tZi + 0.5
                ) + tZi * ((NY - 1) * (tpZ - Zmin) / (Zmax - Zmin) <= tZi + 0.5)
                tZm = (tZi) * ((NY - 1) * (tpZ - Zmin) / (Zmax - Zmin) > tZi + 0.5) + (
                    tZi - 1
                ) * ((NY - 1) * (tpZ - Zmin) / (Zmax - Zmin) <= tZi + 0.5)
            elif tZi < NY - 1:
                tZp = tZi
    ...
    def getBs_disc(self, eq):
        ...
        for coil in self.pccoils:
            ...
            if tvec[0] != 0.0:
                ...
                df = (
                    fm
                    * (np.arcsinh(eq.psi()[tRi, tZp]) - np.arcsinh(eq.psi()[tRi, tZm]))
                    / dZ
                )
            ...
            if tvec[2] != 0.0:
                ...
                df = (
                    fm
                    * (np.arcsinh(eq.psi()[tRp, tZi]) - np.arcsinh(eq.psi()[tRm, tZi]))
                    / dR
                )
    ...
    def jtor_and_mask(self, eq, profiles):
        ...
        self.red_jtor = jtor[
            self.red_idx[0, 0] : self.red_idx[0, 1],
            self.red_idx[1, 0] : self.red_idx[1, 1],
        ]
        self.red_plasma_mask = self.red_jtor > 0

        # separatrix characterization
        self.plasma_zloc = np.stack(
            (
                np.mean(self.Zvec * self.red_plasma_mask, axis=1),
    ...
    def fluxes(self, eq):
        ...
        # plasma-plasma flux:
        plasma_self_flux = (
            self.red_jtor[np.newaxis, np.newaxis, :, :] * self.greenmatrix
        )
        ...
        plasma_coil_ind = (
            self.red_plasma_mask[np.newaxis, :, :]
        ) * self.coil_plasma_greens
