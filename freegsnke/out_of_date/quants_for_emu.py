...
class quants_for_emulation:
    ...
    def __init__(self, eq):
        ...
        self.void_matrix = np.zeros(
            (len(coils_dict.keys()) + 1, len(coils_dict.keys()) + 1)
        )
    ...
    def fluxes(self, eq):
        ...
        plasma_coil_ind = (
            self.red_plasma_mask[np.newaxis, :, :] * self.coil_plasma_greens
        )
    ...
    def quants_out(self, eq, profiles):
        ...
        results["plasma_ind_on_coils"] = np.concatenate(
            (self.coil_plasma_ind, [self.simple_plasma_self_ind])
        )
