...
class optimize_profile:
    ...
    def solve_and_control(self, eq, profile, red_Iy0, dresidual,
        ...
        counter += 1 
        control = ((relncandidatedJ<.1) + (relncandidatedJ>10))*(counter<max_iter)
        plt.figure()
        plt.imshow(self.rebuild_grid_map(candidatedJ))
        plt.colorbar()
        plt.title(str(profile.Ip)+'   '+str(profile.alpha_m)+'   '+str(profile.alpha_n))
        return candidatedJ, control, relncandidatedJ


    def build_jacobian(self, eq, profile, red_Iy0, dresidual,
                             rtol_NK, 
