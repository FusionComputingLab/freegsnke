...
def test_implicit_euler_solver():
    ...
    assert hasattr(solver, "inverse_operator"), "Solver did not pre-calculate inverse operator"
    ...
    assert solver.internal_timestep <= full_timestep, "Internal timestep larger than full timestep"

    solver.set_timesteps(full_timestep, 3*full_timestep) # test if internal_timestep > full_timestep
    assert solver.internal_timestep == solver.full_timestep and solver.n_steps == 1, "Error when internal timestep larger than the full timestep"
...
def test_implicit_euler_solver_d():
    """Tests the implicit_euler_solver_d.
    ...
    solver = implicit_euler_solver_d(M, R, 1, 0.1)

    assert hasattr(solver, "inverse_operator"), "Solver did not pre-calculate inverse operator"
    

    M_update = np.random.randn(N, N)
    S_upate = np.random.randn(N)
    R_update = np.diag(S_upate)

    solver.set_Smatrix(np.diag(R_update))
    assert np.all(solver.Smatrix == R_update), "Did not set S matrix properly"
